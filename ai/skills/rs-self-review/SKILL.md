...
- **The test that passes for the wrong reason.** Perturb the behavior or inspect the exact rejection path. Self-derived fixtures, permissive matchers, mocks, unrelated validation failures, and compound assertions regularly make broken behavior stay green.
...
- **The sibling path you didn't update.** Search for every producer and consumer of the changed field or classification. Compare direct/fallback, streaming/buffered, authenticated/open, single/batch, regional, and provider variants so billing, limits, errors, metrics, and trust rules don't drift.
- **The production-sized input.** Put a realistic upper bound through request paths, queries, migrations, refreshes, and code under locks. Look for full scans, N+1 calls, unbounded collections, retained request bodies, repeated parsing, and work whose cost grows on every write.
- **The trust boundary.** Follow tenant ownership and authoritative fields across headers, signatures, redirects, replay, caches, and database writes. Check paths such as bulk updates and raw SQL that bypass application validation.
- **The operational consequence.** For every new failure or classification, decide whether it should retry, trip health, charge, log, emit a metric, page, or remain quiet. Routine disconnects and expected filtering must not drown anomalous signals; fail-open and fail-closed choices must be deliberate.
- **The rollout claim.** Exercise production-facing config, migration, alert, runbook, fallback, and backwards-compatibility assumptions in their real shape. Re-read the final PR description and code comments after the last fix; stale or overstated claims are review findings.
