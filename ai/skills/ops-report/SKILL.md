...
- **service** (optional): The service to report on (default: `llm-analytics`). Other examples: `feature-flags`, `ingestion`, `capture`
...
- `/ops-report` - daily report for llm-analytics (US, the default)
- `/ops-report llm-analytics --window week` - weekly report
...
- `service` - kebab-case service name, default "llm-analytics"
...
```text
~/dev/richardsolomou/notes/PostHog/ops-reports/{YYYY-MM-DD}/{service}-{window}.md
```
