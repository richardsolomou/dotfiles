...
<A short reply the user can post on the PR thread to push back politely or explain the chosen approach. This reply IS posted under the user's name, so load the `tone` skill with `register: pr-review` and apply those rules to this subsection — but not to the rest of the walkthrough.>
...
**Exception:** the **Suggested reply** subsection (used when Disagreeing or explaining a different fix) *is* posted under the user's name on the PR thread. Load the `tone` skill with `register: pr-review` and apply those rules to that subsection only. Don't apply `tone` to the rest of the walkthrough — it'd flatten the explanations.
