...
def test_parse_date_day_before_month():
    assert parse_date("23 Feb", 2026) == date(2026, 2, 23)


def test_parse_date_strips_ordinal_suffix():
    assert parse_date("22nd Aug", 2026) == date(2026, 8, 22)


def test_parse_date_normalizes_sept():
...
def test_parse_sprint_dates_dash_separated_range():
    """Real titles use a dash between dates, not the word 'to'."""
    start, end = parse_sprint_dates("Sprint - March 23 - April 5", date(2026, 3, 25))
    assert start == date(2026, 3, 23)
    assert end == date(2026, 4, 5)


def test_parse_sprint_dates_dash_same_month():
    start, end = parse_sprint_dates("Sprint - Dec 9 - Dec 20", date(2026, 12, 12))
    assert start == date(2026, 12, 9)
    assert end == date(2026, 12, 20)


def test_parse_sprint_dates_no_leading_dash():
    """Older titles omit the dash after 'Sprint'."""
    start, end = parse_sprint_dates("Sprint April 21 to May 2", date(2026, 4, 25))
    assert start == date(2026, 4, 21)
    assert end == date(2026, 5, 2)


def test_parse_sprint_dates_ordinals_and_sept():
    start, end = parse_sprint_dates("Sprint 22nd Sept - 3rd Oct", date(2026, 9, 25))
