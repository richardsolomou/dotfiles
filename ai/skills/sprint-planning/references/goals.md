...
Steps for `/sprint-planning goals`. They run independently of the main sprint planning workflow; step references such as "Step 1" and "Step 2" point at the main workflow in `SKILL.md`.
...
2. Fetch the team's comment from the current sprint issue:
   ...
   ```bash
   ~/.agents/skills/sprint-planning/scripts/fetch-previous-comment.sh <current_number>
   ```
...
```bash
~/.agents/skills/sprint-planning/scripts/fetch-board-goals.sh
```
