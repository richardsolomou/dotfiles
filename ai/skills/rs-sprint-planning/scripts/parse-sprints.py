...
def normalize_date_str(s):
    """Normalize sprint date fragments to what strptime accepts.

    Strips ordinal suffixes ("23rd" -> "23") and rewrites "Sept" to the
    strptime-recognised "Sep", so titles like "22nd Sept" parse.
    """
...
def parse_date(s, reference_year):
    ...
    Handles both abbreviated and full month names, ordinal suffixes, and
    "Sept". Tries parsing the string as-is first (for titles with an explicit
    year like 'Feb 23 2026'), then falls back to appending the reference year.
    The month may come before the day ("Feb 23") or after it ("23 Feb").
    """
...
def parse_sprint_dates(title, today):
    ...
    Tolerates the separator variants seen in real titles: an optional "-" or
    ":" after "Sprint", and either "to" or a dash ("-", "–", "—") between the
    two dates.
    """
