...
This is the multi-agent counterpart to `rs-review-pr`: it borrows the discipline from `rs-adversarial-review`, and the mode detection, inline-comment format, and orientation from `rs-review-pr`, with the voice from `rs-tone`. It does not restate them — it loads and applies them. Unlike `rs-review-pr` it is not bounded to five findings; coverage is the point.
...
Each pass re-reviews the current HEAD and posts a fresh review. It **never edits the author's branch** — on someone else's PR the only action is commenting; the fixing is the author responding to the comments. Reacting to CI, conflicts, and incoming feedback on your *own* open PR is `rs-pr-shepherd`, not this skill.
