...
- `/quarterly-planning review` — Only produce the past-quarter review (the async 20%). Run Steps 1–4, then Step 5, and stop.
- `/quarterly-planning hogs` — Only generate the private HOGS scaffold for the user to fill in. Run Step 1, then Step 6, and stop.
- `/quarterly-planning pr` — Skip prep and jump to opening/updating the objectives PR from goals the user provides (post-meeting). Run Step 1, then the **Objectives PR Workflow**.
...
```bash
~/.agents/skills/quarterly-planning/scripts/detect-quarter.sh
```
...
```bash
source ~/.agents/skills/quarterly-planning/scripts/config.sh
~/.agents/skills/activity-harvest/scripts/team-merged-prs.sh <username> "$QP_ORG" <cur_start> <cur_end> 500
```
...
Write the PR body with the `update-pr` skill (it owns the template handling, structure, and voice) to a temporary file, never substituted into the shell command. Request review from `QP_BLITZSCALE_REVIEWER` — if it's empty, ask the user who the team's Blitzscale member is before opening.
...
gh pr create --repo "PostHog/posthog.com" \
  ...
  --body-file /tmp/quarterly-pr-body.md $reviewer_flag
rm -f /tmp/quarterly-pr-body.md
```
